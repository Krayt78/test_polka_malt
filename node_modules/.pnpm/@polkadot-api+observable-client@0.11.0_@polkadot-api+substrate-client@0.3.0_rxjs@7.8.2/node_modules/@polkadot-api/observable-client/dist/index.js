...
const withInitializedNumber = (getHeader) => (source$) => new rxjs.Observable((observer) => {
  let pending = null;
...
const getRuntimeCreator = (call$, getCodeHash$, getCachedMetadata, setCachedMetadata) => {
  const getMetadata$ = (getHash) => {
    ...
    const versioned$ = (availableVersions) => {
      ...
      return v === 14 ? recoverCall$("Metadata_metadata", "").pipe(rxjs.map(opaqueBytes.dec)) : recoverCall$("Metadata_metadata_at_version", versionedArgs(v)).pipe(
        rxjs.map((x) => optionalOpaqueBytes.dec(x))
      );
    };
    const metadataRaw$ = versions$.pipe(rxjs.mergeMap(versioned$));
    return recoverCodeHash$().pipe(
      rxjs.mergeMap(
        (codeHash) => getCachedMetadata(codeHash).pipe(
          rxjs.catchError(() => rxjs.of(null)),
          rxjs.mergeMap(
            (metadataRaw) => metadataRaw ? rxjs.of(metadataRaw) : metadataRaw$.pipe(
              rxjs.tap((raw) => {
  ...
  return (getHash) => {
    ...
    const runtimeContext$ = getMetadata$(
      ...
      rxjs.map(({ metadata, metadataRaw, codeHash }) => {
