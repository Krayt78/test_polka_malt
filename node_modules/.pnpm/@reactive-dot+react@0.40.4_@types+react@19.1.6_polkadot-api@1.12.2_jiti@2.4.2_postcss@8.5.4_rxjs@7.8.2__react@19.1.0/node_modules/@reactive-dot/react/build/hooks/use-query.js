...
mayBeOptions) {
    ...
    const fetchKey = mayBeOptions !== undefined && "fetchKey" in mayBeOptions
...
const instructionPayloadAtom = atomFamilyWithErrorCatcher((withErrorCatcher, config, chainId, instruction) => {
    switch (preflight(instruction)) {
        case "promise": {
            const atom = withErrorCatcher(atomWithPromise((get, { signal }) => soon(get(typedApiAtom(config, chainId)), (api) => query(api, instruction, { signal }))));
            return {
                observableAtom: atom,
                promiseAtom: atom,
            };
